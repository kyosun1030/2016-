...
public class WingScript : MonoBehaviour {
    ...
    public bool wingLock = false;       // 잠긴상태인지 아닌지 ture = 잠김 false = 열림
    ...
    // 문을 여는 코루틴을 활성화하는 함수
    public void WingActive(int speed, int angle)
    {
        if (!wingLock)
        {
           if(reverse)
           {
               StartCoroutine(WingOpen(this.gameObject, speed, angle * -1));
           }
           else
           {
               StartCoroutine(WingOpen(this.gameObject, speed, angle));
           }
        }
        else
        {
    ...
    IEnumerator WingOpen(GameObject obj, int num, int angle)
    {
        ...
        {
            yield return null;
            obj.transform.localEulerAngles += new Vector3(0, valueY, 0);
            count++;
        }
